...
public class Ramasseur<T> {
    ...
    // ATTRIBUTS a DEFINIR
    private String name;
    private static final String DEFAULT = "Ramasseur";
	private boolean mainPleine;
	private T objetPorte;

	// Constructeur
	public Ramasseur(String name) {
	this.name = DEFAULT+"-"+name;
	this.mainPleine = false;
    this.objetPorte = null;
    }
    ...
    public void donneA(Ramasseur<? super T > r) throws IllegalStateException {
	if (this.porteObjet()){
	   if (r.porteObjet())
	...
	return (this.name+" porte un objet "+this.objetPorte);
	}


	public boolean porteObjet(){
	return this.mainPleine;
	}
